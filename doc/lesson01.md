...
- 19 При редиректе все атрибуты (`req.getAttribute()`) теряется (см. вопрос выше). Сценарий редиректа:
  - 1 из сервлета делаем редирект
  - 2 снава заходим в сервлет
  - 3 кладем нужные аттрибуты и делаем forward на jsp
  - 4 если очень хочется передать параметры из 1. в 2. можно сделать их через параметры запроса (meals?id=5). В моей реализации такого не потребовалось.
